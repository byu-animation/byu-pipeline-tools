...
def build(
    ...
    if side=="_LFT":
        mc.move(-.40,0.0,0.0, name + side + '_leg_settings_curve_offset_os_grp_01', relative=True)
    else:
        mc.move(.40,0.0,0.0, name + side + '_leg_settings_curve_offset_os_grp_01', relative=True)
    ...
    if side == "_LFT":
        ...
        mc.joint(name=name + side + "_upper_leg_FK_JNT_01", position = mc.xform(hip_joint, query=True, rotatePivot=True, worldSpace=True), absolute=True, radius=0.01)
        mc.parent(name + side + "_upper_leg_FK_JNT_01", name + '_secondary_global_cc_01')

        # create lower_leg FK JNT
        mc.joint(name=name + side + "_lower_leg_FK_JNT_01", position = mc.xform(knee_joint, query=True, rotatePivot=True, worldSpace=True), absolute=True, radius=0.01)
        mc.joint( name + side + '_upper_leg_FK_JNT_01', edit=True, zeroScaleOrient=True, orientJoint='xzy', secondaryAxisOrient = 'zup' )

        # create ankle FK JNT
        mc.joint(name=name + side + "_ankle_FK_JNT_01", position = mc.xform(ankle_joint, query=True, rotatePivot=True, worldSpace=True), absolute=True, radius=0.01)
    ...
    if side == "_LFT":
        ...
        mc.joint(name=name + side + '_foot_ball_FK_JNT_01', radius=.05)
        ...
        mc.joint(name=name + side + '_foot_ball_JNT_01', position = mc.xform(name + side + '_foot_ball_target_cc_01', query=True, rotatePivot=True, worldSpace=True), absolute=True, radius=.05)
        '''
    """
    else:
        #rename foot joints already duplicated in joint chain
    ...
    mc.move(.2*x_scale,0.0,0.0, foot_cupping_control.Off, relative=True)
    mc.move(-.2*x_scale,0.0,0.0, foot_cupping_control.C + '.rotatePivot', relative=True)
    mc.move(-.2*x_scale,0.0,0.0, foot_cupping_control.C + '.scalePivot', relative=True)
    ...
    for toe in ['big', 'index', 'middle', 'fourth', 'pinky', 'second_pinky']:
        ...
        if(mc.objExists(name + side + '_' + toe + '_toe_PRO_target_cc_01')):
            ...
            # proximal phalange
            mc.joint(name=name + side + '_' + toe + '_toe_PRO_BN_01', radius=.05)
            ...
            if side == "_LFT":
                ...
                mc.move(0.0,0.07,.10, relative=True)
            else:
                mc.select( name + side + '_' + toe + '_toe_primary_cc_01Shape.cv[0:4]' )
                mc.rotate(0.0,0.0,0.0, relative=True)
                mc.scale(3.0,1.0,0.5, relative=True)
            ...
            if(mc.objExists(name + side + '_' + toe + '_toe_MED_target_cc_01')):
                mc.joint(name=name + side + '_' + toe + '_toe_MED_BN_01', radius=.05)
            ...
            if(mc.objExists(name + side + '_' + toe + '_toe_DIS_target_cc_01')):
                mc.joint(name=name + side + '_' + toe + '_toe_DIS_BN_01', radius=.05)
            ...
            if(mc.objExists(name + side + '_' + toe + '_toe_DIS_2_target_cc_01')):
                mc.joint(name=name + side + '_' + toe + '_toe_DIS_2_BN_01', radius=.05)
            ...
            if(mc.objExists(name + side + '_' + toe + '_toe_DIS_3_target_cc_01')):
                mc.joint(name=name + side + '_' + toe + '_toe_DIS_3_BN_01', radius=.05)
            ...
            mc.joint(name=name + side + '_' + toe + '_toe_JNT_END_01', radius=.05)
    ...
    for part in ['_hip_bendy_cc', '_upper_leg_bendy_cc', '_knee_bendy_cc', '_lower_leg_bendy_cc', '_ankle_bendy_cc']:
        mc.addAttr(name + side + part + '_01', shortName='noodle_parameter', longName='Noodle', attributeType='float', defaultValue=noodle_default, minValue=0.0, maxValue=10.0, keyable=True)
    ...
    for num in range(1, number_of_upper_leg_joints+number_of_lower_leg_joints+1):
        ...
        if num <= number_of_upper_leg_joints:
            mc.joint(name=name + side + '_leg_bendy_'+str(num)+'_JNT_01', position = mc.xform(name + side + '_upper_leg_stretchy_target_'+str(num)+'_cc_01', query=True, rotatePivot=True, worldSpace=True), absolute=True, radius=0.05)
        else:
            mc.joint(name=name + side + '_leg_bendy_'+str(num)+'_JNT_01', position = mc.xform(name + side + '_lower_leg_stretchy_target_'+str(num-number_of_upper_leg_joints)+'_cc_01', query=True, rotatePivot=True, worldSpace=True), absolute=True, radius=0.05)
    #reorient chain
    ...
    for num in range(1, number_of_upper_leg_joints+number_of_lower_leg_joints+1):
        # create upper_leg bendy JNTs
        if num <= number_of_upper_leg_joints:
            mc.joint(name=name + side + '_leg_bendy_pointer_'+str(num)+'_JNT_01', position = mc.xform(name + side + '_upper_leg_stretchy_target_'+str(num)+'_cc_01', query=True, rotatePivot=True, worldSpace=True), absolute=True, radius=0.05)
        else:
            mc.joint(name=name + side + '_leg_bendy_pointer_'+str(num)+'_JNT_01', position = mc.xform(name + side + '_lower_leg_stretchy_target_'+str(num-number_of_upper_leg_joints)+'_cc_01', query=True, rotatePivot=True, worldSpace=True), absolute=True, radius=0.05)
    #reorient chain
    ...
    for num in range(1, number_of_upper_leg_joints+number_of_lower_leg_joints+1):
        ...
        if num <= number_of_upper_leg_joints:
            mc.joint(name=name + side + '_leg_bendy_'+str(num)+'_BN_01', position = mc.xform(name + side + '_upper_leg_stretchy_target_'+str(num)+'_cc_01', query=True, rotatePivot=True, worldSpace=True), absolute=True, radius=0.05)
        else:
            mc.joint(name=name + side + '_leg_bendy_'+str(num)+'_BN_01', position = mc.xform(name + side + '_lower_leg_stretchy_target_'+str(num-number_of_upper_leg_joints)+'_cc_01', query=True, rotatePivot=True, worldSpace=True), absolute=True, radius=0.05)
    #reorient chain
    ...
    #For DANG BENT LEGS only:
    if name=='Grendel' or name=='Dragon':
        knee_pos=mc.xform(knee_joint, query=True, rotatePivot=True, worldSpace=True)
    ...
    #setup to get joints to scale with splineIK curve
    #find splineIK curve scale length
    mc.shadingNode('multiplyDivide', asUtility=True, name= name + side + '_leg_splineIK_initial_pointer_curve_length_multiplied_by_rig_scale_MD_01')
    ...
    mc.setAttr( name + side + '_leg_splineIK_initial_pointer_curve_length_multiplied_by_rig_scale_MD_01.input1X', splineIK_start_pointer_curve_length)
    mc.connectAttr( name + '_secret_total_scale_MD_01.outputX', name + side + '_leg_splineIK_initial_pointer_curve_length_multiplied_by_rig_scale_MD_01.input2X')

    mc.connectAttr( curveInfoNode + '.arcLength', name + side + '_leg_splineIK_pointer_curve_scale_length_MD_01.input1X' )
    mc.connectAttr( name + side + '_leg_splineIK_initial_pointer_curve_length_multiplied_by_rig_scale_MD_01.outputX', name + side + '_leg_splineIK_pointer_curve_scale_length_MD_01.input2X')
    ...
    mc.shadingNode('multiplyDivide', asUtility=True, name=name + side + '_leg_splineIK_pointer_curve_simplify_compensate_input_MD_01')
    mc.connectAttr( name + side + '_leg_splineIK_pointer_curve_simplify_compensate_MD_01.outputX', name + side + '_leg_splineIK_pointer_curve_simplify_compensate_input_MD_01.input1X')
    mc.connectAttr( name + side + '_leg_splineIK_pointer_curve_scale_length_MD_01.outputX', name + side + '_leg_splineIK_pointer_curve_simplify_compensate_input_MD_01.input2X')
    ...
    mc.setAttr( name + side + '_leg_splineIK_pointer_curve_joint_scale_MD_01.operation', 1)
    ...
    for num in range(1, number_of_upper_leg_joints+number_of_lower_leg_joints+1):
        ...
        mc.connectAttr( name + side + '_leg_splineIK_pointer_curve_simplify_compensate_input_MD_01.outputX', name + side + '_leg_bendy_pointer_'+str(num)+'_JNT_01.scaleX', force=True)
