...
def build(
    ...
    mc.addAttr( neck_settings_control.C, shortName='stretch_volume_parameter', longName='Stretch_Volume', attributeType='float', defaultValue=1.0, keyable=True)
    ...
    mc.move(-1.4,0.0,0.0, name + '_neck_settings_curve_offset_os_grp_01', relative=True)
    ...
    neck_base_BN=mc.joint(name=name + "_neck_base_BN_01", position = mc.xform(neck_base_target, query=True, rotatePivot=True, worldSpace=True), absolute=True, radius=.05)
    mc.parent(neck_base_BN, name + '_secondary_global_cc_01')

    # create lower neck BN
    mc.select(clear=True)
    lower_neck_BN=mc.joint(name=name + "_LOW_neck_BN_01", position = mc.xform(lower_neck_target, query=True, rotatePivot=True, worldSpace=True), absolute=True, radius=.05)

    # create middle neck BN
    middle_neck_BN=mc.joint(name=name + "_MID_neck_BN_01", position = mc.xform(middle_neck_target, query=True, rotatePivot=True, worldSpace=True), absolute=True, radius=.05)

    # create upper neck BN
    upper_neck_BN=mc.joint(name=name + "_UPP_neck_BN_01", position = mc.xform(upper_neck_target, query=True, rotatePivot=True, worldSpace=True), absolute=True, radius=.05)

    # create head JNT
    head_JNT=mc.joint(name=name + "_head_JNT_01", position = mc.xform(head_target, query=True, rotatePivot=True, worldSpace=True), absolute=True, radius=.05)
    ...
    #keep things simple for the crazy bent dragon neck
    if name != 'Dragon':
        #make spline IK
        ...
        upploc=mc.spaceLocator(name=name + '_neck_upper_twist_loc_01')[0]
        lowloc=mc.spaceLocator(name=name + '_neck_lower_twist_loc_01')[0]

        #align locators to controls with offset
        mc.delete(mc.parentConstraint(head_JNT, upploc, maintainOffset=False))
        mc.delete(mc.parentConstraint(neck_base_BN, lowloc, maintainOffset=False))
        mc.move(0,10,0, upploc, objectSpace=True, relative=True)
        mc.move(0,10,0, lowloc, objectSpace=True, relative=True)

        #parenting
        mc.parent(name + '_neck_splineIK_01', name + '_neck_splineIK_curve_01', upploc, lowloc, name + '_neck_extras_GRP_01')
        ...
        for part in ['LOW', 'MID', 'UPP']:
            ...
            mc.shadingNode('plusMinusAverage', asUtility=True, name = name + '_neck_' + part + '_length_minus_one_PMA_01')
            mc.setAttr(name + '_neck_' + part + '_length_minus_one_PMA_01.operation', 2)
            mc.setAttr(name + '_neck_' + part + '_length_minus_one_PMA_01.input3D[1].input3Dy', 1.0 )
            mc.setAttr(name + '_neck_' + part + '_length_minus_one_PMA_01.input3D[1].input3Dz', 1.0 )

            mc.shadingNode('multiplyDivide', asUtility=True, name= name + '_neck_' + part + '_input_multiplied_by_stretch_volume_attr_MD_01')

            mc.shadingNode('multiplyDivide', asUtility=True, name= name + '_neck_' + part + '_input_multiplied_by_stretch_volume_individual_adjustment_MD_01')
            if name == 'Viking':
                if part=='LOW':
                    mc.setAttr(name + '_spine_' + part + '_input_multiplied_by_stretch_volume_individual_adjustment_MD_01.input2Y', 1.2)
                    mc.setAttr(name + '_spine_' + part + '_input_multiplied_by_stretch_volume_individual_adjustment_MD_01.input2Z', 1.2)
                elif part == 'MID':
                    mc.setAttr(name + '_spine_' + part + '_input_multiplied_by_stretch_volume_individual_adjustment_MD_01.input2Y', 1.2)
                    mc.setAttr(name + '_spine_' + part + '_input_multiplied_by_stretch_volume_individual_adjustment_MD_01.input2Z', 1.2)
                elif part =='UPP':
            ...
            mc.shadingNode('plusMinusAverage', asUtility=True, name= name + '_neck_' + part + '_input_plus_one_PMA_01')
            mc.setAttr(name + '_neck_' + part + '_input_plus_one_PMA_01.input3D[1].input3Dy', 1.0 )
            mc.setAttr(name + '_neck_' + part + '_input_plus_one_PMA_01.input3D[1].input3Dz', 1.0 )

            mc.shadingNode('multiplyDivide', asUtility=True, name= name + '_neck_' + part + '_one_divided_by_input_MD_01')
            mc.setAttr(name + '_neck_' + part + '_one_divided_by_input_MD_01.operation', 2)
            mc.setAttr(name + '_neck_' + part + '_one_divided_by_input_MD_01.input1Y', 1.0 )
            mc.setAttr(name + '_neck_' + part + '_one_divided_by_input_MD_01.input1Z', 1.0 )


            #connections
            mc.connectAttr(name + '_neck_splineIK_curve_scale_length_MD_01.outputX', name + '_neck_' + part + '_length_minus_one_PMA_01.input3D[0].input3Dy', force=True)
            mc.connectAttr(name + '_neck_splineIK_curve_scale_length_MD_01.outputX', name + '_neck_' + part + '_length_minus_one_PMA_01.input3D[0].input3Dz', force=True)

            mc.connectAttr(name + '_neck_' + part + '_length_minus_one_PMA_01.output3Dy', name + '_neck_' + part + '_input_multiplied_by_stretch_volume_attr_MD_01.input1Y', force=True)
            mc.connectAttr(name + '_neck_' + part + '_length_minus_one_PMA_01.output3Dz', name + '_neck_' + part + '_input_multiplied_by_stretch_volume_attr_MD_01.input1Z', force=True)
            mc.connectAttr(neck_settings_control.C + '.stretch_volume_parameter', name + '_neck_' + part + '_input_multiplied_by_stretch_volume_attr_MD_01.input2Y', force=True)
            mc.connectAttr(neck_settings_control.C + '.stretch_volume_parameter', name + '_neck_' + part + '_input_multiplied_by_stretch_volume_attr_MD_01.input2Z', force=True)

            mc.connectAttr(name + '_neck_' + part + '_input_multiplied_by_stretch_volume_attr_MD_01.outputY', name + '_neck_' + part + '_input_multiplied_by_stretch_volume_individual_adjustment_MD_01.input1Y', force=True)
            mc.connectAttr(name + '_neck_' + part + '_input_multiplied_by_stretch_volume_attr_MD_01.outputZ', name + '_neck_' + part + '_input_multiplied_by_stretch_volume_individual_adjustment_MD_01.input1Z', force=True)

            mc.connectAttr(name + '_neck_' + part + '_input_multiplied_by_stretch_volume_individual_adjustment_MD_01.outputY', name + '_neck_' + part + '_input_plus_one_PMA_01.input3D[0].input3Dy', force=True)
            mc.connectAttr(name + '_neck_' + part + '_input_multiplied_by_stretch_volume_individual_adjustment_MD_01.outputZ', name + '_neck_' + part + '_input_plus_one_PMA_01.input3D[0].input3Dz', force=True)

            mc.connectAttr(name + '_neck_' + part + '_input_plus_one_PMA_01.output3Dy', name + '_neck_' + part + '_one_divided_by_input_MD_01.input2Y', force=True)
            mc.connectAttr(name + '_neck_' + part + '_input_plus_one_PMA_01.output3Dz', name + '_neck_' + part + '_one_divided_by_input_MD_01.input2Z', force=True)

            mc.connectAttr(name + '_neck_' + part + '_one_divided_by_input_MD_01.outputY', name + '_' + part + '_neck_BN_01.scaleY', force=True)
            mc.connectAttr(name + '_neck_' + part + '_one_divided_by_input_MD_01.outputZ', name + '_' + part + '_neck_BN_01.scaleZ', force=True)
    ...
    if name != 'Dragon':
        mc.parent(name + '_neck_splineIK_curve_01', name + '_neck_no_transform_GRP_01')
    ...
    if name != 'Dragon':
        ...
        mc.scaleConstraint(neck_base_control.C, name + '_neck_base_CLU_01', maintainOffset=True)
        mc.scaleConstraint(LOW_neck_control.C, name + '_LOW_neck_CLU_01', maintainOffset=True)
        mc.scaleConstraint(MID_neck_control.C, name + '_MID_neck_CLU_01', maintainOffset=True)
        mc.scaleConstraint(UPP_neck_control.C, name + '_UPP_neck_CLU_01', maintainOffset=True)
        mc.scaleConstraint(head_control.C, name + '_head_CLU_01', maintainOffset=True)
    elif name == 'Dragon':
        mc.pointConstraint(neck_base_control.C, name + '_neck_base_BN_01', maintainOffset=True)
        mc.pointConstraint(LOW_neck_control.C, name + '_LOW_neck_BN_01', maintainOffset=True)
        mc.pointConstraint(MID_neck_control.C, name + '_MID_neck_BN_01', maintainOffset=True)
        mc.pointConstraint(UPP_neck_control.C, name + '_UPP_neck_BN_01', maintainOffset=True)
        mc.pointConstraint(head_control.C, name + '_head_BN_01', maintainOffset=True)
        mc.orientConstraint(neck_base_control.C, name + '_neck_base_BN_01', maintainOffset=True)
        mc.orientConstraint(LOW_neck_control.C, name + '_LOW_neck_BN_01', maintainOffset=True)
        mc.orientConstraint(MID_neck_control.C, name + '_MID_neck_BN_01', maintainOffset=True)
        mc.orientConstraint(UPP_neck_control.C, name + '_UPP_neck_BN_01', maintainOffset=True)
        mc.orientConstraint(head_control.C, name + '_head_BN_01', maintainOffset=True)
    ...
    if name == 'Dragon':
        mc.setAttr(name + '_neck_settings_cc_01.Secondaries_Visibility', 1.0)
