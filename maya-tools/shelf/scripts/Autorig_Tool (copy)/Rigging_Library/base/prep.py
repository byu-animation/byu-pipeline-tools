...
def set_project_variables(selected_project):
    ...
    main_project_path = '/Users/Erik/Documents/Pipe/characters'
    geometry_filepath = '%s/%s/model/main/%s_model_main.mb'
    builder_scene_filepath = '%s/%s/autorig/builder/%s_builder.mb'
    primary_targets_filepath = '%s/%s/autorig/primary_targets/%s.mb'
    secondary_targets_filepath = '%s/%s/autorig/secondary_targets/%s.mb'
...
def mirror_primary_rig_targets_button_press(GUI_window, builder_scene_filepath, main_project_path, scene_scale, maya_button_trash):
    ...
    #eyebrows
    for part in ['_INN', '_MID', '_OUT']:
        proper_xform=mc.xform(character_name + side_to_mirror + part + '_eyebrow_target_cc_01', query=True, translation=True, relative=True)
        mc.setAttr(character_name + mirrored_side + part + '_eyebrow_target_cc_01.translateX', proper_xform[0] * -1.0 )
        mc.setAttr(character_name + mirrored_side + part + '_eyebrow_target_cc_01.translateY', proper_xform[1] )
        mc.setAttr(character_name + mirrored_side + part + '_eyebrow_target_cc_01.translateZ', proper_xform[2] )
    ...
    #uniquely name everything
    transform_objects=[]
    shape_objects=[]

    mc.select('*_target_cc_01*')

    transform_objects=mc.ls(selection=True, type='transform')
    #control_objects=mc.listRelatives(control_transforms, children=True, shapes=True)

    for transform_object in transform_objects:
        print('transform_object='+transform_object)
        shape_objects = mc.listRelatives( transform_object, fullPath=True, shapes=True )
        if shape_objects != None:
            for shape_object in shape_objects:
                print('shape_object='+shape_object)
                mc.rename(shape_object, transform_object+'shape'+str(shape_objects.index(shape_object)+1))
    for transform_object in transform_objects:
        print('transform_object='+transform_object)
        shape_objects = mc.listRelatives( transform_object, shapes=True )
        if shape_objects != None:
            for shape_object in shape_objects:
                print('shape_object='+shape_object)
                mc.rename(shape_object, transform_object+'shape'+str(shape_objects.index(shape_object)+2))

    #recolor target curves
    mc.select('*target_cc_01')
    control_objects=mc.ls(selection=True, type='transform')
    #control_objects=mc.listRelatives(control_transforms, children=True, shapes=True)

    for control_object in control_objects:
        print('control_object='+control_object)
        control_shapes = mc.listRelatives( control_object, shapes=True )
        print(control_shapes)
        for control_shape in control_shapes:
            print(control_shape)
            if mc.objExists(control_shape + '.overrideEnabled'):
...
def build_rig_button_press(GUI_window, builder_scene_filepath, main_project_path, character_name, maya_button_trash):
    ...
    # re-unlock channels
    mc.select('*finger*')
    if mc.objExists('*thumb*'):
        mc.select('*thumb*', add=True)
    ...
    #############################TEMP for getting past student license glitch
...
def initialize_builder_scene(which_pass):
    ...
    has_thumbs = mc.checkBox( 'has_thumbs', query = True, value=True )
    ...
    if which_pass == 'primary':
        ...
        for side in ["_LFT" , "_RGT"]:
            ...
            for phalange in thumb_phalanges:
                thumb_target = control.Control(
                              ...
                              scale = 0.06,
                              use_numerical_transforms = True,
                              transform_x = .10 * x_scale * (thumb_phalanges.index(phalange)+3),
                              transform_y = -0.06,
                              transform_z = .24,
                              parent = hand_target.C,
                              shape = 'spherical_target',
            ...
            for finger in fingers:
                ...
                metacarpal_target = control.Control(
                                                  ...
                                                  scale = 0.06,
                                                  use_numerical_transforms = True,
                                                  transform_x = (.10 * x_scale),
                                                  transform_y = 0.0,
                                                  transform_z = (.125-((fingers.index(finger)*.15+.1)/2.0)),
                                                  parent = hand_target.C,
                                                  shape = 'spherical_target',
                                                  locked_channels = ['visibility']
                                                  )
                ...
                for phalange in finger_phalanges:
                    finger_target = control.Control(
                                                  ...
                                                  scale = 0.06,
                                                  use_numerical_transforms = True,
                                                  transform_x = (10.0 * x_scale * (finger_phalanges.index(phalange)+1)/100)+.2*x_scale,
                                                  transform_y = 0.0,
                                                  transform_z = (.125-((fingers.index(finger)*.15+.1)/2.0)),
                                                  parent = hand_target.C,
                                                  shape = 'spherical_target',
            ...
            for toe in toes:
                for phalanx in toe_phalanges:
                    toe_target = control.Control(
                                              ...
                                              scale = 0.06,
                                              use_numerical_transforms = True,
                                              transform_x = 0.05 * (toe_phalanges.index(phalanx)+1),
                                              transform_y = 0.0,
                                              transform_z = (-.125+((toes.index(toe)/10.0+.1)/2.0)),
                                              parent = foot_ball_target.C,
                                              shape = 'spherical_target',
                                              locked_channels = ['visibility']
                                              )
        ...
        def make_stretchy_joint_targets( side, part, character_name, number_of_joints, scale, parent, end_target ):
            ...
            for stretchy_joint_number in range(1, number_of_joints+2):
                #prevent elbow duplicates, defer to lower arm/leg
                if stretchy_joint_number==1:
                    if (part=='upper_arm' or part=='upper_leg'):
        ...
        if has_tail:
            ...
            mc.delete(mc.parentConstraint(character_name + '_tail_base_target_cc_01', character_name + '_tail_MID_target_cc_01', upper_tail_secondary_target.Off, maintainOffset=False))
            mc.delete(mc.parentConstraint(character_name + '_tail_MID_target_cc_01', character_name + '_tail_tip_target_cc_01', lower_tail_secondary_target.Off, maintainOffset=False))
...
def make_GUI(character_name, scene_scale, main_project_path, geometry_filepath, builder_scene_filepath, primary_targets_filepath, secondary_targets_filepath, skin_weights_directory, skin_weights_extension):
    ...
    mc.text("Has thumbs: ")
    has_thumbs = mc.checkBox( "has_thumbs", label='Has Thumbs' )
    ...
    mc.intField( "number_of_joints_per_toe", edit=True, changeCommand = ('mc.setFocus("number_of_upper_arm_spans")') )
    ...
    mc.intField( "number_of_lower_leg_spans", edit=True, changeCommand =  ('mc.setFocus("initialize_button")') )
