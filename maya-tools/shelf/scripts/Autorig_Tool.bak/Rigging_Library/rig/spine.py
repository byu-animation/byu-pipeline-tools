...
def build(
    ...
    mc.move(-.14,0.0,0.0, name + '_spine_settings_curve_offset_os_grp_01', relative=True)
    ...
    root_JNT=mc.joint(name=name + "_root_JNT_01", position = mc.xform(root_target, query=True, translation=True, worldSpace=True), absolute=True, radius=.01)
    mc.parent(root_JNT, name + '_secondary_global_cc_01')
    
    # create lower belly BN
    mc.select(clear=True)
    lower_belly_BN=mc.joint(name=name + "_LOW_belly_BN_01", position = mc.xform(lower_belly_target, query=True, translation=True, worldSpace=True), absolute=True, radius=.01)
    
    # create middle belly BN
    middle_belly_BN=mc.joint(name=name + "_MID_belly_BN_01", position = mc.xform(middle_belly_target, query=True, translation=True, worldSpace=True), absolute=True, radius=.01)
    
    # create upper belly BN
    upper_belly_BN=mc.joint(name=name + "_UPP_belly_BN_01", position = mc.xform(upper_belly_target, query=True, translation=True, worldSpace=True), absolute=True, radius=.01)
    
    # create chest JNT
    chest_JNT=mc.joint(name=name + "_chest_JNT_01", position = mc.xform(lower_ribcage_target, query=True, translation=True, worldSpace=True), absolute=True, radius=.01)
    
    # create chest BN
    chest_BN=mc.joint(name=name + "_chest_BN_01", position = mc.xform(lower_ribcage_target, query=True, translation=True, worldSpace=True), absolute=True, radius=.01)
    
    # create hip BN
    mc.select(root_JNT)
    hips_BN=mc.joint(name=name + "_hips_BN_01", position = mc.xform(root_target, query=True, translation=True, worldSpace=True), absolute=True, radius=.01)
    ...
    mc.parent(name + '_spine_splineIK_01', name + '_spine_splineIK_curve_01', name + '_spine_extras_GRP_01')
