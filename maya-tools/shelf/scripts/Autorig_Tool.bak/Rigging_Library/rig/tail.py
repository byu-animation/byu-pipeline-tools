...
def build(
    ...
    tail_base_BN=mc.joint(name=name + "_tail_base_BN_01", position = mc.xform(tail_base_target, query=True, translation=True, worldSpace=True), absolute=True, radius=.01)
    mc.parent(tail_base_BN, name + '_secondary_global_cc_01')
    
    # create upper tail BN
    upper_tail_BN=mc.joint(name=name + "_UPP_tail_BN_01", position = mc.xform(upper_tail_target, query=True, translation=True, worldSpace=True), absolute=True, radius=.01)
    
    # create middle tail BN
    middle_tail_BN=mc.joint(name=name + "_MID_tail_BN_01", position = mc.xform(middle_tail_target, query=True, translation=True, worldSpace=True), absolute=True, radius=.01)
    
    # create lower tail BN
    lower_tail_BN=mc.joint(name=name + "_LOW_tail_BN_01", position = mc.xform(lower_tail_target, query=True, translation=True, worldSpace=True), absolute=True, radius=.01)
    
    # create tail_tip JNT
    tail_tip_JNT=mc.joint(name=name + "_tail_tip_JNT_01", position = mc.xform(tail_tip_target, query=True, translation=True, worldSpace=True), absolute=True, radius=.01)
    
    # create tail_tip BN
    tail_tip_BN=mc.joint(name=name + "_tail_tip_BN_01", position = mc.xform(tail_tip_target, query=True, translation=True, worldSpace=True), absolute=True, radius=.01)
