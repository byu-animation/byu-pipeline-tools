...
def build(
    ...
    mc.move(-1.4,0.0,0.0, name + '_head_settings_curve_offset_os_grp_01', relative=True)
    ...
    mc.joint(name=name + "_jaw_BN_01", position = mc.xform(name + '_jaw_target_cc_01', query=True, rotatePivot=True, worldSpace=True), absolute=True, radius=.05)
    ...
    mc.setAttr(cluster + '.scaleY', .70)
    mc.setAttr(cluster + '.translateZ', 2.2)
    mc.setAttr(cluster + '.translateY', -1.7)
    ...
    mc.joint(name=name + "_UPP_teeth_BN_01", position = (0.0,0.0,0.0), absolute=True, radius=.05)
    ...
    mc.joint(name=name + "_LOW_teeth_BN_01", position = (0.0,0.0,0.0), absolute=True, radius=.05)
    ...
    mc.select(name + "_jaw_BN_01")
    mc.joint(name=name + "_tongue_root_BN_01", position = (0.0,0.0,0.0), absolute=True, radius=.05)
    ...
    mc.joint(name=name + "_tongue_middle_BN_01", position = (0.0,0.0,0.0), absolute=True, radius=.05)
    ...
    mc.joint(name=name + "_tongue_tip_BN_01", position = (0.0,0.0,0.0), absolute=True, radius=.05)
    ...
    mc.setAttr(cluster + '.scaleX', .70)
    mc.setAttr(cluster + '.rotateX', 90)
    ...
    mc.select(clear=True)
    mc.joint(name=name + "_LFT_eye_BN_01", position = mc.xform(name + '_LFT_eye_target_cc_01', query=True, rotatePivot=True, worldSpace=True), absolute=True, radius=.05)
    mc.joint(name=name + "_LFT_eye_JNT_END_01", position = (.3,0.0,0.0), relative=True, radius=.05)
    ...
    for side in ["_LFT_", "_RGT_"]:
        ...
        mc.move(0.0,0.0,3.0, eye_aim_control.Off, relative=True, worldSpace=True)
        ...
        mc.setAttr(cluster + '.translateZ', .8)
    ...
    for side in ["_LFT", "_RGT"]:
        ...
        mc.setAttr( name + side + '_UPP_eyelid_control_translate_MD_01.input2X', -30.0)

        mc.shadingNode('multiplyDivide', asUtility=True, name= name + side + '_LOW_eyelid_control_translate_MD_01')
        mc.setAttr( name + side + '_LOW_eyelid_control_translate_MD_01.input2X', 30.0)
        ...
        mc.setAttr(cluster + '.translateZ', .8)
        mc.setAttr(cluster + '.translateY', .1)
    ...
    for side in ["_LFT", "_RGT"]:
        for part in ["INN", "MID", "OUT"]:
            ...
            elif mc.objExists(name + '_body_GEO_0Shape1.outMesh'):
            ...
            print('closest_vertex='+str(closest_vertex))
            #mc.polySelectConstraint( pp=1 )
            mc.softSelect(softSelectEnabled=True,softSelectDistance=0.5*rig_scale,softSelectFalloff=1)
